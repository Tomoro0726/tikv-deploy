...
# 1. スワップの無効化
echo "[1] スワップ領域を無効化しています..."
swapoff -a
sed -i -e '/swap/d' /etc/fstab
sed -i -e '/none.*swap/d' /etc/fstab

# 2. CPUガバナーの設定
echo "[2] CPUガバナーを performance に設定しています..."
...
echo "[3] limits.conf と sysctl の設定を適用しています..."
...
echo "[4] Transparent Huge Pages (THP) を無効化しています..."
...
# 5. 時刻同期（NTP）の設定
echo "[5] chrony (NTP) をインストールして設定しています..."
DEBIAN_FRONTEND=noninteractive apt-get install -y -qq chrony
systemctl enable chrony
systemctl restart chrony

# 6.irqbalanceのインストール
echo "[6] irqbalanceをインストールして設定しています..."
